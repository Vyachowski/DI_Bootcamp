...
class TheIncredibles(Family):
  ...
  def use_power(self, name):
    if self.is_18(name):
      for member in self.members: 
        if name in member.values(): print(member['power']) 
    else: raise Exception('Sorry, no superpowers until 18 years old')

  def incredible_presentation(self):
    self.family_presentation()
    print(f'It is {self.name} Incredible Family!\n\nFrom left to right:')
    for member in self.members: print(f'{member["incredible_name"]} can {member["power"]}')
...
petrovi = TheIncredibles(incredibles, 'Petrovi')
print(petrovi.members)
petrovi.use_power('Sarah')
petrovi.incredible_presentation()
petrovi.born(name='Jack', gender='Male', power = 'Unknown_Power', incredible_name="Unknown")
print(petrovi.members)
petrovi.incredible_presentation()
